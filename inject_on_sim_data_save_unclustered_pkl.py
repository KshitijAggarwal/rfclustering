...
def inject_one(preffile, devicenum, outdir):
    ...
    st = state.State(inmeta=meta, showsummary=False, preffile=preffile, name='NRAOdefault'+band, inprefs=prefs)
    segment = 0
    data = source.read_segment(st, segment)
    dmind = None
    dtind = None
    snr = np.random.uniform(low=10, high=40)
    mock = util.make_transient_params(st, snr = snr, segment=segment, data = data, ntr = 1, 
                                      lm = -1, dmind=dmind, dtind=dtind)
    ...
    if not len(cc):
        logging.info('No candidate found. Deleting the empty pickle, and trying with a higher SNR now.')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Inject simulated FRB on simulated data and save"
    ...
    inject_one(preffile=values.preffile, devicenum=str(values.devicenum), outdir=values.outdir)
