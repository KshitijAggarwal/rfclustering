...
def vis_cands(cc):
    """
    To visualize all candidates in a 4 panel plot
    ...
    i_peakx_ind, i_peaky_ind = cc.state.calcpix(
        np.array(i_l), np.array(i_m), npixx, npixy, uvres
    )
...
def plot_data(data, labels=[], snrs=[], tsne_res=None, title=None, save=False):
    """
    Visualize clustered candidates using a 4 panel plot with 2 feature each.
    ...
    if len(labels):
        ...
        ]  # assigning each cluster a color, and making a list

        cluster_colors = list(
            map(colors.rgb2hex, cluster_colors)
        )  # converting sns colors to hex for bokeh
    else:
    ...
    ax[0, 0].scatter(
        data[:, 0], data[:, 1], color=cluster_colors, alpha=0.5, s=10 * snr
    )
    ax[0, 0].set_xlabel("l")
    ax[0, 0].set_ylabel("m")
    ax[0, 0].grid()
    ...
    ax[0, 1].scatter(
        data[:, 2], data[:, 3], color=cluster_colors, alpha=0.5, s=10 * snr
    )
    ...
    ax[1, 0].scatter(
        data[:, 0], data[:, 3], color=cluster_colors, alpha=0.5, s=10 * snr
    )
    ax[1, 0].set_xlabel("l")
    ...
    ax[1, 1].scatter(
        data[:, 0], data[:, 2], color=cluster_colors, alpha=0.5, s=10 * snr
    )
    ax[1, 1].set_xlabel("l")
    ...
    if tsne_res:
        ax[2, 1].scatter(
            tsne_res[:, 0], tsne_res[:, 1], color=cluster_colors, s=10 * snr
        )
        ax[2, 1].set_xlabel("x")
        ax[2, 1].set_ylabel("y")
        ax[2, 1].grid()
...
def vis_clustering(data, cc, labels, tsne_res=None, title=None):
    """
    Old plotting function to visualize clustering
