...
def process(sdm, gainpath, preffile, devicenum, outdir):
    ...
    gainpath = gainpath #'/hyrule/data/users/kshitij/hdbscan/gainfiles/'
    preffile = preffile #'/hyrule/data/users/kshitij/hdbscan/scripts/final/realfast_nocluster.yml'
    ...
    datasetId = '.'.join([ x for x in datasetId.split('.') if "scan" not in x and 'cut' not in x])
    ...
    pipeline_sdm(sdm, inprefs=prefs, intent='TARGET', preffile=preffile, devicenum=devicenum)
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Normal pipeline search on an SDM",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-f', '--file', help='Path of SDM', required=True, type=str)
    parser.add_argument('-d', '--devicenum', help='GPU numbers', required=False, type=int, default=0)
    parser.add_argument('-g', '--gainpath', help='Path of gain files', required=False, type=str, 
                        default='/hyrule/data/users/kshitij/hdbscan/gainfiles/')
    parser.add_argument('-p', '--preffile', help='Path of preffile', required=False, type=str, 
    ...
    process(sdm=sdm, gainpath=values.gainpath, preffile=values.preffile, devicenum=str(values.devicenum)
